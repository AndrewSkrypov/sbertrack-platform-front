...
class AgentService(
    ...
    fun sendMessage(sessionId: UUID, request: AgentMessageRequest): AgentSessionResponse {
        ...
        val contextText = buildContextText(request)
        val assistantText = contextText + agentGatewayPort.generateAssistantResponse(
            agent,
            withUserMessage.messages,
            listOfNotNull(request.caseTitle?.let { "Кейс: $it" }, request.content).joinToString(". "),
        )
